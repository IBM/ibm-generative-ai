...
#   filename:  2024-03-21_openapi_schema
...
class ModerationHAP(ApiBaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if deprecated_attrs and deprecated_attrs.get("threshold") is None:
            deprecated_attrs["threshold"] = 0.75  # TODO: remove before deploy
        for attr in deprecated_attrs:
            _log_deprecation_warning(
                f"genai.schema.ModerationHAP.{attr}",
                f"Property '{attr}' is deprecated! Use 'input' and 'output' property.",
            )
            self.input = self.input or ModerationHAPInput.model_validate(deprecated_attrs)
            self.output = self.output or ModerationHAPOutput.model_validate(deprecated_attrs)
            delattr(self, attr)

    @field_validator("input", mode="before")
    @classmethod
    def validate_input(cls, value: Any):
        return ModerationHAPInput(enabled=value, threshold=0.75) if isinstance(value, bool) else value

    @field_validator("output", mode="before")
    @classmethod
    def validate_output(cls, value: Any):
        return ModerationHAPOutput(enabled=value, threshold=0.75) if isinstance(value, bool) else value
...
class ModerationHAPInput(ApiBaseModel):
    ...
    """
    send_tokens: Optional[bool] = False
    threshold: Optional[float] = Field(0, ge=0.01, le=0.99, multiple_of=0.01)
    """
...
class ModerationImplicitHate(ApiBaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        for attr in deprecated_attrs:
            _log_deprecation_warning(
                f"genai.schema.ModerationImplicitHate.{attr}",
                f"Property '{attr}' is deprecated! Use 'input' and 'output' property.",
            )
            self.input = self.input or ModerationImplicitHateInput.model_validate(deprecated_attrs)
            self.output = self.output or ModerationImplicitHateOutput.model_validate(deprecated_attrs)
            delattr(self, attr)

    @field_validator("input", mode="before")
    @classmethod
    def validate_input(cls, value: Any):
        return ModerationImplicitHateInput(enabled=value, threshold=0.75) if isinstance(value, bool) else value

    @field_validator("output", mode="before")
    @classmethod
    def validate_output(cls, value: Any):
        return ModerationImplicitHateOutput(enabled=value, threshold=0.75) if isinstance(value, bool) else value
...
class ModerationParameters(ApiBaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        for name in ["stigma", "implicit_hate"]:
            if hasattr(self, name):
                _log_deprecation_warning(
                    f"genai.schema.ModerationParameters.{name}",
                    f'"{name}" has been deprecated, use "social_bias" instead.',
                )
                delattr(self, name)

    @field_validator("hap", mode="before")
    @classmethod
    def validate_hap(cls, value: Any):
        return (
            ModerationHAP(
                input=ModerationHAPInput(enabled=value, threshold=0.75),
                output=ModerationHAPOutput(enabled=value, threshold=0.75),
            )
    ...
    @classmethod
    def validate_social_bias(cls, value: Any):
        return (
            ModerationSocialBias(
                input=ModerationSocialBiasInput(enabled=value, threshold=0.75),
                output=ModerationSocialBiasOutput(enabled=value, threshold=0.75),
            )
            if isinstance(value, bool)
            else value
        )
...
class ModerationStigma(ApiBaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        for attr in deprecated_attrs:
            _log_deprecation_warning(
                f"genai.schema.ModerationStigma.{attr}",
                f"Property '{attr}' is deprecated! Use 'input' and 'output' property.",
            )
            self.input = self.input or ModerationStigmaInput.model_validate(deprecated_attrs)
            self.output = self.output or ModerationStigmaOutput.model_validate(deprecated_attrs)
            delattr(self, attr)

    @field_validator("input", mode="before")
    @classmethod
    def validate_input(cls, value: Any):
        return ModerationStigmaInput(enabled=value, threshold=0.75) if isinstance(value, bool) else value

    @field_validator("output", mode="before")
    @classmethod
    def validate_output(cls, value: Any):
        return ModerationStigmaOutput(enabled=value, threshold=0.75) if isinstance(value, bool) else value
